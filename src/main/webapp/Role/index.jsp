...
<body>
<form class="form-inline definewidth m20" action="/getroles" method="get">
    角色名称：
    <input type="text" name="rName" id="rName"class="abc input-default" placeholder="" value="${rName}">&nbsp;&nbsp;
    ...
    <c:forEach items="${pageInfo.list}" var="r">
	     <tr>
         	<td style="vertical-align:middle;">
                <input type="checkbox" name="check" value="${r.rId}"></td>
            <td>${r.rName}</td>
            <td>${r.rState==0?'启用':'禁用'}</td>
            <td><a href="/findrolebyid?rid=${r.rId}">编辑</a>
                &nbsp;&nbsp;&nbsp;<a href="deleterole?rid=${r.rId}">删除</a> </td>
        </tr>
    </c:forEach>
</table>
